...
public class MejoresJuegos {
	...
	public static void addValoracion(Integer gameId, Float valoracion) {
		/*
		 * Construimos una tabla con la valoracion media de cada juego Key :
		 * Integer - El identificador del juego Value : Pair<Integer, Float> -
		 * Un par que indica el A) el numero de usuarios que han valorado este
		 * juego B) la valoracion media
		 */

		// Si el juego no existe en la tabla de valoraciones media (TVM) se
		// introduce
		if (!tvm.containsKey(gameId))
			tvm.put(gameId, new Pair<Integer, Float>(1, valoracion));
		else {
			/*
			 * Recalculamos la valoracion media para el juego dado newV = ((oldV
			 * * n) + v) / (n+1)
			 */
			Pair<Integer, Float> entry = tvm.get(gameId);
			entry.elem1 = ((entry.elem1 * entry.elem0) + valoracion)
					/ (entry.elem0 + 1);
	...
	public static ArrayList<CBRCase> getMejoresJuegos(int n) {
		...
		ColaPrioridad<ComparablePair<Float, Integer>> colaMJ = new ColaPrioridad<ComparablePair<Float, Integer>>(
				n);

		Set<Entry<Integer, Pair<Integer, Float>>> tabla = tvm.entrySet();
		/*
		 * Por cada entrada de la tabla de valoraciones media introducimos el
		 * par <Valoracion, gameID> en la cola, esta se encarga de mirar si es
		 * realmente necesario introducir el par o no.
		 */
		for (Entry<Integer, Pair<Integer, Float>> entry : tabla) {
			ComparablePair<Float, Integer> cp = new ComparablePair<Float, Integer>(
					entry.getValue().elem1, entry.getValue().elem0);
			colaMJ.add(cp);
		}

		// Convertimos la cola de prioridad en un ArrayList de <Float, Integer>
		ArrayList<ComparablePair<Float, Integer>> listaMJ = colaMJ
				.toArrayList();
		...
		for (CBRCase c : Sistema.getCBjuegosInstance().getCases()) {
			// Si el identificador coincide con alguno de los del array de
			// mejores juegos lo metemos
...
class ComparablePair<T1 extends Comparable, T2> extends Pair<T1, T2> implements
		Comparable {
