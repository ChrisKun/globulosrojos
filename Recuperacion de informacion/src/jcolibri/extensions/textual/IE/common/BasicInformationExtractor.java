...
public class BasicInformationExtractor
{
	...
	try
	{
	    ...
	    // Nombre del atributo - Lista de características
	    ...
	    for (int i = 0; i < attrs.length; i++)
	    {
			...
			if (o instanceof CaseComponent)
				...
				lists.put(attrs[i].getName(), (List<String>) o);
			else
			    other.add(attrs[i]);
	    ...
	    {
	    	List<FeatureInfo> fInfo = text.getFeatures();
	    	phrases.addAll(text.getPhrases());
	    	features.addAll(fInfo);
	    	for (FeatureInfo feature : fInfo)
				if (lists.containsKey(feature.getFeature()) &&
						!lists.get(feature.getFeature()).contains(feature.getValue()))
					lists.get(feature.getFeature()).add(feature.getValue());
	    }
